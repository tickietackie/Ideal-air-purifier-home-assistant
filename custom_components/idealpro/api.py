...
# How long to wait for the status answer to the GD wake-up (seconds).
# The device usually answers within ~250 ms but can take over a second when busy.
HANDSHAKE_TIMEOUT = 2.5
# Keep the connection open after a command so the device can apply it (seconds)
COMMAND_POST_DELAY = 0.5
# Grace period used to drain late data before closing a socket (seconds).
...
class IdealProAPI:
    ...
    @staticmethod
    async def _drain(reader, timeout: float = CLOSE_DRAIN_TIMEOUT) -> None:
        """Read and discard late data so close() sends FIN instead of RST."""
        loop = asyncio.get_running_loop()
        deadline = loop.time() + timeout
        try:
            while True:
    ...
    async def _execute(self, command: bytes) -> None:
        """
        Send a command the way the vendor app does.

        The device silently drops commands that arrive while it is still
        waking up, or that are sent on a connection that is closed too fast.
        Therefore:
        1. connect and send the GD handshake,
        2. wait for the complete status answer - this is what actually wakes
           the device up (~250 ms, sometimes over a second),
        3. send the command on the same, now fully awake connection,
        4. keep the socket open briefly and drain late data before closing.
        """
        ...
        try:
            writer.write(b"GD")
            await writer.drain()
            status = await self._read_status(reader, HANDSHAKE_TIMEOUT)
            if not status:
                _LOGGER.debug("No answer to GD wake-up, sending command anyway")
    ...
    async def _handshake_and_read(self, timeout: float = 2.0) -> Optional[str]:
        """Connect, send GD handshake and read the status block (if any)."""
        reader, writer = await self._connect()
        try:
            writer.write(b"GD")
            await writer.drain()
            data = await self._read_status(reader, timeout)
            await self._drain(reader)
            return data
        finally:
            await self._close(writer)
    ...
    async def async_handshake_and_read(self, timeout: float = 2.0) -> Optional[str]:
        ...
        async with self._lock:
            return await self._handshake_and_read(timeout)

    async def _get_power_state(self) -> Optional[str]:
        """
    ...
    async def async_get_power_state(self) -> Optional[str]:
        ...
        """
        async with self._lock:
            return await self._get_power_state()
    ...
    async def _turn_on(self, max_retries: int = MAX_RETRIES) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            # Check current state
            current_state = await self._get_power_state()
    ...
    async def async_set_brightness(self, level: int):
        ...
        async with self._lock:
            await self._execute(f"D{level}".encode())

    async def _get_led_level(self) -> Optional[int]:
        """
    ...
    async def async_get_led_level(self) -> Optional[int]:
        ...
        """
        async with self._lock:
            return await self._get_led_level()

    async def _set_brightness_verified(self, target_level: int, max_retries: int = MAX_RETRIES) -> bool:
        """Set LED brightness with state verification (lock must be held)."""
        ...
        for attempt in range(max_retries):
            ...
            # Check current level
            current_level = await self._get_led_level()
            ...
            try:
                await self._execute(f"D{target_level}".encode())
            except Exception as err:
            ...
            new_level = await self._get_led_level()
    ...
    async def _get_fan_speed(self) -> Optional[str]:
        """
    ...
    async def async_get_fan_speed(self) -> Optional[str]:
        ...
        """
        async with self._lock:
            return await self._get_fan_speed()
    ...
    async def async_set_fan_speed(self, mode: str):
        ...
        async with self._lock:
            await self._execute(self.FAN_SPEED_COMMANDS[mode])
    ...
    async def _set_fan_speed_verified(self, target_mode: str, max_retries: int = MAX_RETRIES) -> bool:
        ...
        if target_mode in self.UNVERIFIABLE_MODES:
            ...
            for attempt in range(max_retries):
                try:
                    await self._execute(self.FAN_SPEED_COMMANDS[target_mode])
                    await asyncio.sleep(POST_TOGGLE_DELAY)
        ...
        for attempt in range(max_retries):
            ...
            # Check current speed
            current_speed = await self._get_fan_speed()
            ...
            new_speed = await self._get_fan_speed()
